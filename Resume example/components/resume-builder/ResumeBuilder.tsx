...
const ResumeBuilder: React.FC = () => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = Object.keys(resume?.sectionsOrder || {}).indexOf(active.id as string);
    const newIndex = Object.keys(resume?.sectionsOrder || {}).indexOf(over.id as string);
    
    if (oldIndex !== -1 && newIndex !== -1) {
      const sections = Object.keys(resume?.sectionsOrder || {});
      const newSections = arrayMove(sections, oldIndex, newIndex);
      
      const newOrder = newSections.reduce((acc, key, index) => ({
        ...acc,
        [key]: index + 1
      }), {});
